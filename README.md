...
> Florence-2 (Tier-1) + High-Inference VLM (Tier-2) + Shadow/Evolution + LoRA 데이터 수집
...
- `Tier2Validator`: Tier-2 검증 (`validate_event_evidence`)
...
|            /monitor/adhoc  /monitor/shadow  /monitor/validation-logs            |
+-----------------------------------+----------------------------------------------+
                                    |
                                    | /api/vlm/*  (HTTP Proxy)
                                    v
...
|  | - events/clips/thumb |      | periodic /api/flush |     | events/valid_logs | |
|  | - FFmpeg H.264       |      +----------------------+     +-------------------+ |
|  +----------------------+                                                     ^    |
|                                                                                |    |
|  +----------------------+   +----------------------+   +---------------------+ |    |
...
|                                                                                |    |
|  +----------------------+                                                       |    |
|  | LoRA DataCollector   |  (cash + tier2 validated + clip frames only)        |    |
|  +----------------------+                                                       |    |
+----------------------------------------------------------------------------------+
```
...
- `Tier-2 검증 메타 DB 적재`로 운영 추적 강화 (`validation_logs`)
...
| Tier 2: High-Inference VLM | `Tier2Validator.validate_event_evidence()` | mode: `hybrid/video_first/video_only/images_first` |
...
5. 필요 시 High-Inference VLM 검증 (`validate_event_evidence`)
6. Tier-2 거부 시 이벤트 저장 스킵
7. 승인 이벤트만 이벤트/클립/썸네일 저장
8. flush worker가 DB로 주기적 동기화

### Tier-2 관련 동작

- `state=skipped`: Tier-1에서 충분히 확신하여 Tier-2 호출 생략
- `state=done + validated=true`: 검증 통과
- `state=done + validated=false`: 검증 거부(저장 스킵)
- `state=error`: Tier-2 오류 시 fail-open (운영 가용성 우선)
...
### 3) 검증 로그 (`/monitor/validation-logs`)
...
- 검증 로그 파일(로컬): `data/validation_logs/*`
...
- 검증 로그: `http://localhost:8002/monitor/validation-logs`
...
- 모델
  ...
  - `TIER2_MODEL=high-inference-vlm`
- 임계값
...
- Tier-2 검증 로그 전용 DB 조회 API 추가(현재는 이벤트 기반 조회 중심)
